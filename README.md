...
Typography follows content roles: Barlow Condensed for chapter, story and card headings; DM Sans for paragraphs; monospace only for short metadata labels. Daily fragments may use standalone `h2.chapter` headings or `section.chapter` containers with a direct `.section-kicker`. The renderer normalizes containers into sections with real chapter headings before enhancing them, preserving all articles, paragraphs, links and visuals. Story `h2` headings become `h3`; the introduction supplies the cover headline. Signal cards use `.signal-title` headings and separate paragraphs, with optional `.signal-label` metadata. Both historical `strong` + `span` + `p` and newer `strong` + `br` + text cards are supported. Never apply display typography or replace `textContent` on an entire chapter container.
...
With Playwright and Chromium available, run `node tests/typography.cjs` against that preview (`BRIEF_URL` overrides its URL). The regression check covers every date in `data/archive.json` in DE/EN at four widths and verifies content/link preservation, heading roles, font separation, cover/navigation labels and overflow.
