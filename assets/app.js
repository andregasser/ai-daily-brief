...
async function loadBrief() {
  ...
  try {
    ...
    $('#cover-visual').replaceChildren(editorialNode('div','cover-placeholder',language === 'de' ? 'Wir bleiben dran.' : 'Stay curious.'));
    $('#cover-kicker').textContent = 'AI DAILY BRIEF';
    $('#hero-date').textContent = requestedDate || '—';
    $('#brief-date').textContent = requestedDate || '—';
    $('#edition-signals').hidden = true;
...
async function loadArchive() {
  ...
  try {
    ...
    const rows = items.map((item, index) => {
      ...
      link.href = `./?date=${encodeURIComponent(item.date)}&lang=${language}#brief`;
